...
const DonateForm = ({ isOpen, onClose, bloodRequest, onPledgeSubmitted }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Pledge Donation for: {bloodRequest.bloodTypeNeeded}</DialogTitle>
          <DialogDescription>
            Thank you for your willingness to help! Please confirm your details.
            The hospital location is: <strong>{bloodRequest.hospitalLocation}</strong>.
            {bloodRequest.contactPhone && <> You can also contact them at: <strong>{bloodRequest.contactPhone}</strong></>}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Your Contact Phone *</FormLabel>
                  <FormControl>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    ...
                    <SelectContent>
                      {["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-", "Unknown"].map((type) => (
                        <SelectItem key={type} value={type}>{type === "Unknown" ? "I don't know my blood type" : type}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  <FormMessage />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Quantity to Pledge (units) *</FormLabel>
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Notes for Hospital (Optional)</FormLabel>
                  <FormControl>
                    <Textarea placeholder="e.g., Availability, previous donor ID..." {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
