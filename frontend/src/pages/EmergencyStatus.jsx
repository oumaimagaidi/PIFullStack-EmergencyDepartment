...
const EmergencyStatus = () => {
   ...
   // Remplacer la condition dans fetchMedicalAccessCode
if (!patientDetails || 
    !['Médecin En Route', 'Traité'].includes(patientDetails.status)) { // 👈 Utiliser les libellés français
    return;
}
    ...
    const renderMedicalAccessCode = () => {
        ...
        return (
            <div className="border rounded-md p-6 bg-red-50 mt-4"> {/* Changed bg-purple-50 to bg-red-50 */}
              <h4 className="font-semibold mb-4 flex items-center">
                <Clipboard className="mr-2 h-5 w-5 text-red-600" /> {/* Changed text-purple-700 to text-red-600 */}
                Your Medical Access Code
              </h4>
              <div className="bg-white p-4 rounded-md border border-red-200"> {/* Changed border-purple-200 to border-red-200 */}
                <p className="text-center text-2xl font-bold text-red-600"> {/* Changed text-purple-800 to text-red-600 */}
                  {medicalAccess.code}
                </p>
                <p className="text-sm text-gray-500 mt-2 text-center">
                  Keep this code safe to access your medical records
                </p>
              </div>
              <div className="mt-4 text-sm text-gray-600">
                <p>This code will allow you to:</p>
                <ul className="list-disc pl-5 mt-2 space-y-1">
                  <li>View your medical records online</li>
                  <li>Share your information with professionals</li>
                            ...
                            <div className="flex flex-col sm:flex-row justify-between gap-4 mt-6">
                                ...
                                {patientDetails.status !== 'Treated' && patientDetails.status !== 'Canceled' && (
                                    <Button asChild variant="default" className="bg-blue-600 hover:bg-blue-700 text-white">
                                        <Link to="/document">
                                            <Calendar className="mr-2 h-4 w-4" /> see yor document record
                                        </Link>
                                    </Button>
                                )}
                            </div>
