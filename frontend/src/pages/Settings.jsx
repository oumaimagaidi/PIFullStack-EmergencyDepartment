...
// Import Icons
import { SettingsIcon, Volume2, VolumeX, AlertCircle, Loader2 } from 'lucide-react'
...
const Settings = () => {
      ...
      try {
        ...
        console.log("Settings: Profile settings state initialized", {
          ...
          localProfileSettings: {
            accessibility: { ttsEnabled: initialTTSActive, ttsLanguage: initialTTSLanguage },
          },
        })
      } catch (err) {
  ...
  const handleTTSwitchChange = (checked) => {
    ...
    setProfileSettings((prev) => {
      const newState = {
        // Use a function to ensure we get the latest 'prev' state
        ...prev,
        accessibility: {
  ...
  if (!Cookies.get("token")) {
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <Alert variant="warning" className="bg-[#DDA853]/20 border-[#DDA853]/30 text-[#213448]">
          <AlertCircle className="h-4 w-4 text-[#DDA853]" />
          <AlertDescription>
            You need to be logged in to save settings to your profile. Accessibility preferences are currently saved
        ...
        <Card className="shadow-lg mt-6 border-[#94B4C1]/30 dark:bg-[#213448]">
          <CardHeader>
            <CardTitle className="text-[#213448] dark:text-[#ECEFCA]">Accessibility Settings (Local)</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 p-6">
            <div className="space-y-4 p-4 border rounded-lg bg-[#ECEFCA]/10 dark:bg-[#213448]/50 border-[#94B4C1]/30">
              <div className="flex items-center justify-between p-3 border rounded-md bg-white dark:bg-[#213448] border-[#94B4C1]/30">
                <div className="space-y-0.5">
                  <Label htmlFor="tts-switch-local" className="text-base text-[#213448] dark:text-[#ECEFCA]">
                ...
                <Switch
                  ...
                  className="data-[state=checked]:bg-[#547792]"
                />
              </div>
              <div className="flex items-center justify-between p-3 border rounded-md bg-white dark:bg-[#213448] border-[#94B4C1]/30">
                <Label htmlFor="tts-language-local" className="text-base text-[#213448] dark:text-[#ECEFCA]">
                  Language for Speech
                </Label>
                <Select value={ttsLanguage} onValueChange={setTtsLanguage}>
                  {" "}
                  // Uses/Updates global state directly
                  <SelectTrigger
                    id="tts-language-local"
                    className="w-[180px] dark:bg-[#213448] border-[#94B4C1]/30 dark:border-[#547792] dark:text-[#ECEFCA]"
                  >
                    <SelectValue placeholder="Select language" />
                  </SelectTrigger>
                  <SelectContent className="dark:bg-[#213448] border-[#94B4C1]/30">
                    {availableLanguages.map((lang) => (
                      <SelectItem key={lang.value} value={lang.value} className="dark:text-[#ECEFCA]">
                        {lang.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center h-64">
        <Loader2 className="h-8 w-8 animate-spin text-[#547792]" />
        <span className="ml-2 text-[#547792]">Loading settings...</span>
      </div>
    )
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      {error && ( // Display the non-blocking error (e.g., fetch failed but showed cached data, or save failed)
        <Alert variant="warning" className="bg-[#DDA853]/20 border-[#DDA853]/30 text-[#213448] mb-6">
          <AlertCircle className="h-4 w-4 text-[#DDA853]" />
          <AlertDescription>{error}</AlertDescription>
        </Alert>
      )}

      <Card className="shadow-lg border-[#94B4C1]/30 dark:bg-[#213448]">
        <CardHeader className="border-b border-[#94B4C1]/30 dark:border-[#547792]">
          <CardTitle className="flex items-center gap-2 text-xl text-[#213448] dark:text-[#ECEFCA]">
            <SettingsIcon className="h-6 w-6 text-[#547792]" /> General Settings
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-6 p-6">
          {/* Section Accessibilité */}
          <div className="space-y-4 p-4 border rounded-lg bg-[#ECEFCA]/10 dark:bg-[#213448]/50 border-[#94B4C1]/30">
            <h3 className="text-lg font-semibold text-[#213448] dark:text-[#ECEFCA] flex items-center gap-2">
              <Volume2 className="h-5 w-5 text-[#547792] dark:text-[#94B4C1]" /> Accessibility
            </h3>
            <div className="flex items-center justify-between p-3 border rounded-md bg-white dark:bg-[#213448] border-[#94B4C1]/30">
              <div className="space-y-0.5">
                <Label htmlFor="tts-switch" className="text-base text-[#213448] dark:text-[#ECEFCA]">
                  Enable Text-to-Speech on Hover/Focus
                </Label>
                <p className="text-sm text-[#547792] dark:text-[#94B4C1]">
                  Enables vocal feedback when hovering over or focusing interactive elements.
            ...
            <div className="flex items-center justify-between p-3 border rounded-md bg-white dark:bg-[#213448] border-[#94B4C1]/30">
              <Label htmlFor="tts-language" className="text-base text-[#213448] dark:text-[#ECEFCA]">
                Language for Speech
              </Label>
              <Select value={currentTTSLanguage} onValueChange={handleLanguageSelectChange}>
                {" "}
                // Uses/Updates the calculated state
                <SelectTrigger
                  id="tts-language"
                  className="w-[180px] dark:bg-[#213448] border-[#94B4C1]/30 dark:border-[#547792] dark:text-[#ECEFCA]"
                >
          ...
          <div className="space-y-4 p-4 border rounded-lg bg-[#ECEFCA]/10 dark:bg-[#213448]/50 border-[#94B4C1]/30">
            <h3 className="text-lg font-semibold text-[#213448] dark:text-[#ECEFCA]">Other Settings</h3>
            <p className="text-sm text-[#547792] dark:text-[#94B4C1]">... Add other settings forms here ...</p>
          </div>
        </CardContent>
        <CardFooter className="flex justify-end p-6 border-t border-[#94B4C1]/30 dark:border-[#547792]">
          <Button
            onClick={handleSaveSettings}
            disabled={isSaving}
            className="bg-[#547792] hover:bg-[#213448] text-[#ECEFCA]"
          >
